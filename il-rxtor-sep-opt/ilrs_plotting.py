...
def draw_qpu_graph(qpu, result):
    """
    This function draws the graph of the QPU and the embedding of the solution (result).

    Parameters
    ----------
    qpu : DWaveSampler
        The D-Wave QPU sampler.
    result : dimod.SampleSet
        The result of the quantum annealing solver.
    """
...
def plot_energies(results, imported=False, title=None, skip=1, n_sig_digit=2, xl_skip=5, log_x=False):
    """
    Plot the energies of the results of the optimization.
    ...
    imported : bool, optional
        Whether the data is imported as a Pandas DataFrame, by default False
    title : str, optional
    ...
    num_sig_digits = n_sig_digit  # Adjust this to your desired significant digits
    skip_x = xl_skip  # Customize how frequently you want labels displayed

    # Plot only a subset of xlabels (every skip steps) and format the numbers
    ax.set_xticklabels([f"{float(t.get_text()):.{num_sig_digits}g}" if not i % skip_x else "" 
...
def plot_enumerate(results, imported=False, title=None):
    """
    Plot the enumerated results of the optimization.
    ...
    if imported:
        # Handle `results` as a Pandas DataFrame
        if 'energy' in results.columns:
            energies = results['energy'].tolist()
        else:
            raise ValueError("DataFrame must contain an 'energy' column")

        if 'sample' in results.columns:
            samples = results['sample'].tolist()
            plt.xlabel('bitstring for solution')
        else:
            samples = np.arange(len(energies))
            plt.xlabel('solution')
    else:
        # Handle `results` as dimod or neal sample object
